...
from .github_fetcher import (
    parse_github_url,
    fetch_pr as fetch_github_pr,
    fetch_issue as fetch_github_issue,
    build_review_context as build_github_review_context,
)
from .atomgit_fetcher import (
    parse_atomgit_url,
    fetch_pr as fetch_atomgit_pr,
    fetch_issue as fetch_atomgit_issue,
    build_review_context as build_atomgit_review_context,
)
...
class VirtualReviewRunner:
    ...
    def _ensure_configured(self):
        ...
        if not any(
            model_name.startswith(p) for p in ("gemini/", "openai/", "anthropic/", "claude")
        ):
        ...
        sub_model = SUB_MODEL
        if not any(sub_model.startswith(p) for p in ("gemini/", "openai/", "anthropic/", "claude")):
            sub_model = f"gemini/{sub_model}"
    ...
    async def review(self, url: str, question: str) -> tuple[str, list[str], dict]:
        """Review a GitHub or AtomGit URL (PR or Issue).

        Args:
            url: GitHub or AtomGit PR/Issue URL
        ...
        if platform == "atomgit":
            ...
            # Fetch content
            if url_type == "pr":
                data = await fetch_atomgit_pr(owner, repo, number, url)
            else:
                data = await fetch_atomgit_issue(owner, repo, number, url)
            # Build context
            context = build_atomgit_review_context(data)
        else:
            owner, repo, number, url_type = parse_github_url(url)
            # Fetch content
            if url_type == "pr":
                data = await fetch_github_pr(owner, repo, number)
            else:
                data = await fetch_github_issue(owner, repo, number)
            # Build context
            context = build_github_review_context(data)
